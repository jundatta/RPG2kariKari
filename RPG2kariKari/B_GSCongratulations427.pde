...
class GameSceneCongratulations427 extends GameSceneCongratulationsBase {
  PShader sd;
  int startMillis;
  int startCount;
  ...
  @Override void setup() {
    ...
    sd = loadShader("data/427/Shadertoy.glsl");
    sd.set("iResolution", (float)width, (float)height, 0.0f);
    // 最初のミリ秒を取り込んでおく
    startMillis = millis();
    startCount = frameCount;
    sd.set("iChannel0", loadImage("data/427/iChannel0.png"));
  }
  @Override void draw() {
    push();
