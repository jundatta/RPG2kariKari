...
class GameSceneCongratulations330 extends GameSceneCongratulationsBase {
  PShader sd;
  int startMillis;
  ...
  @Override void setup() {
    ...
    sd = loadShader("data/330/Shadertoy.glsl");
    sd.set("iResolution", (float)width, (float)height, 0.0f);
    // 最初のミリ秒を取り込んでおく
    startMillis = millis();
    sd.set("iChannel0", loadImage("data/330/iChannel0.png"));
  }
  @Override void draw() {
    push();
